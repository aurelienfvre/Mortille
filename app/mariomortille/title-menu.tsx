...
type Props = { onPrologue: () => void; onStart: (level: number) => void; onExit: () => void; music: number; effects: number; onMusic: (value: number) => void; onEffects: (value: number) => void; onSound: (cue: 'move' | 'confirm' | 'back') => void; savedLevel: number | null; completed: string[]; unlocked: number };
export default function TitleMenu({ onPrologue, onStart, onExit, music, effects, onMusic, onEffects, onSound, savedLevel, completed, unlocked }: Props) {
 const [page, setPage] = useState<'main' | 'campaign' | 'help' | 'options' | 'scores'>('main');
 ...
 const actions = [onPrologue, () => switchPage('campaign'), () => { if (savedLevel !== null) onStart(savedLevel); }, () => switchPage('scores'), () => switchPage('help'), () => switchPage('options'), onExit];
 ...
 return <div ref={root} role="menu" tabIndex={-1} className={styles.menu} aria-label="Menu de Mariomortille" onKeyDown={event => {
  ...
  if (page !== 'main' && page !== 'campaign') return;
  const count = page === 'main' ? labels.length : quartierLevels.length + 1;
  if (['ArrowDown', 'ArrowUp', 'z', 'Z', 's', 'S', ...(page === 'campaign' ? ['ArrowLeft', 'ArrowRight'] : [])].includes(event.key)) {
   event.preventDefault(); let next = (selected + (['ArrowUp','ArrowLeft','z','Z'].includes(event.key) ? -1 : 1) + count) % count;
   if (page === 'main' && next === 2 && savedLevel === null) next = (next + (['ArrowUp','ArrowLeft','z','Z'].includes(event.key) ? -1 : 1) + count) % count;
  ...
  {page === 'main' ? <nav className={styles.choices} aria-label="Menu principal">{labels.map((label, i) => <button key={label} ref={node => { buttons.current[i] = node; }} className={selected === i ? styles.selected : ''} disabled={i === 2 && savedLevel === null} aria-label={label} onFocus={() => setSelected(i)} onMouseEnter={() => { if (i !== 2 || savedLevel !== null) setSelected(i); }} onClick={() => { onSound('confirm'); actions[i](); }}><PixelText text={label} /></button>)}</nav> : <section className={`${styles.panel} ${page === 'campaign' ? styles.campaignPanel : ''}`} aria-label={{ campaign: 'Campagne', help: 'Comment jouer', options: 'Options', scores: 'Classements' }[page]}>
   <h2><PixelText text={{ campaign: 'CAMPAGNE', help: 'COMMENT JOUER', options: 'OPTIONS', scores: 'CLASSEMENTS' }[page]} /></h2>
   {page === 'campaign' && <>
    <CampaignMap selected={selected} unlocked={unlocked} completed={completed} idleFrame={idleFrame} onSelect={setSelected} onStart={onStart} onSound={onSound} buttonRef={(i,node)=>{buttons.current[i]=node;}} />
    <div className={styles.mapCaption} aria-live="polite"><b>{selected < quartierLevels.length ? quartierLevels[selected].title : 'Retour au menu'}</b><span>{selected < quartierLevels.length ? selected > unlocked ? 'Termine l’étape précédente pour ouvrir ce chemin.' : completed.includes(quartierLevels[selected].id) ? 'Terminé — Entrée pour rejouer' : 'Entrée pour commencer' : `${completed.length} / ${quartierLevels.length} étapes terminées`}</span></div>
   </>}
   {page === 'help' && <dl className={styles.help}><dt>← → / Q D</dt><dd>Se déplacer</dd><dt>↑ / ESPACE</dt><dd>Sauter</dd><dt>MAJ</dt><dd>Courir plus vite</dd><dt>X</dt><dd>Utiliser le pouvoir équipé</dd><dt>↓ en l’air</dt><dd>Écraser et casser les blocs fragiles</dd><dt>ÉCHAP</dt><dd>Mettre en pause</dd><p>Ramasse un équipement : son pouvoir s’équipe immédiatement.</p></dl>}
   ...
   <button className={styles.back} ref={node => { if (page === 'campaign') buttons.current[quartierLevels.length] = node; }} onFocus={() => { if (page === 'campaign') setSelected(quartierLevels.length); }} onClick={() => { switchPage('main'); onSound('back'); }}>← RETOUR</button>
  </section>}
  <footer className={styles.footer}><PixelText text={menuFooter} /><span className={styles.srOnly}>Flèches pour choisir, Entrée pour valider. La souris et le tactile fonctionnent aussi.</span></footer>
 </div>;
}
