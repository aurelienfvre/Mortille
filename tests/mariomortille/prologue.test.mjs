...
import { prologueLevel as level, prologueHints, prologueMechanics as mechanics, prologueEncounters } from '../../app/mariomortille/prologue-level.ts';
import { createState, tick, HEIGHT, WIDTH } from '../../app/mariomortille/simulation.ts';
assert.equal(level.id, 'prologue');
assert.equal(level.enemies.length, 11);
assert.ok(level.enemies.every(e => e.left >= 3072), 'initial walking and jumping lessons remain free from enemies');
assert.equal(new Set(level.enemies.map(e => e.id)).size, 11);
assert.equal(prologueEncounters.length, 6);
for (const enemy of level.enemies) {
  assert.ok(Math.abs(enemy.left - level.checkpoint) > 250, 'checkpoint has a safe approach');
  for (let x = enemy.left; x <= enemy.right + 20; x += 4) {
    assert.ok(level.tiles.some(t => t.x <= x && t.x + 16 > x && t.y === enemy.y + 20), 'entire patrol has solid support');
    assert.ok(!level.tiles.some(t => x < t.x + 16 && x + 20 > t.x && enemy.y < t.y + 16 && enemy.y + 20 > t.y), 'patrol body never clips an obstacle');
  }
}
for (const p of level.pickups) assert.ok(!level.tiles.some(t => p.x < t.x + 16 && p.x + 16 > t.x && p.y < t.y + 16 && p.y + 16 > t.y), `${p.id}: reward stays outside obstacles`);
...
function traverse({ missFirstPowers = false, missGlide = false } = {}) {
  ...
  for (let frame = 0; frame < 180 * 60 && !s.won; frame++) {
    ...
    const obstruction = s.tiles.some(t => (t.kind === 'brick' || t.kind === 'ice') && t.x >= p.x + WIDTH - .01 && t.x < p.x + WIDTH + 30 && t.y < feet && t.y + 16 > p.y);
    const enemyAhead = s.enemies.some(e => !e.defeated && e.x > p.x && e.x - p.x < 64 && Math.abs(e.y + 20 - feet) < 20);
    jumpPressed = p.grounded && (step !== undefined && step < feet - 3 || obstruction || enemyAhead);
    // The first guard teaches an aimed stomp: track it during the jump and
    // release horizontal movement above it, using only normal controls.
    const firstGuard = s.enemies[0];
    if (!firstGuard.defeated && p.x > firstGuard.x - 80 && p.x < firstGuard.x + 60 && !p.grounded && feet < firstGuard.y) {
      run = false;
      const distance = firstGuard.x + 2 + firstGuard.direction * 5 - p.x - p.vx * .14;
      direction = Math.abs(distance) < 5 ? 0 : Math.sign(distance);
    }
    for (const start of [mechanics.weakFloor, mechanics.cobaltFloor]) {
      if (!floorsBroken.has(start) && p.x > start - 48 && p.x < start + 128) {
        ...
        downPressed = !p.grounded && p.y < 212 && Math.abs(target - p.x) < 12;
      }
    }
    ...
    for (const start of [mechanics.weakFloor, mechanics.cobaltFloor]) if (!s.tiles.some(t => t.x === start + 64 && t.y === 272)) floorsBroken.add(start);
    if (p.power === 'cloud' && p.x >= mechanics.cloudGap.from && p.x < mechanics.cloudGap.to && p.vy > 0 && p.vy <= 85 && !p.grounded) glideTicks++;
    if (p.x > mechanics.cloudGap.from && p.x < mechanics.cloudGap.to && p.grounded && p.y + HEIGHT > 272) basinLanding = true;
...
assert.ok(s.enemies.filter(e => e.defeated).length >= 2, 'real route engages and defeats tutorial guards');
assert.ok(s.enemies[0].defeated, 'the first guard was stomped using aimed jump input');
...
console.log(`Prologue recovery: ${(recovery.s.ticks / 60).toFixed(2)}s with four first power pickups deliberately skipped and backups recovered; ${(basin.s.ticks / 60).toFixed(2)}s with missed glide, safe basin exit; both zero hurt.`);
