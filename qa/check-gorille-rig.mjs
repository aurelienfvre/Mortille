import {MeshoptDecoder} from 'three-stdlib'; const decoder=typeof MeshoptDecoder==='function'?MeshoptDecoder():MeshoptDecoder;
...
const results=[];
for(const name of ['gorille-mortizle-v1']){
 const b=fs.readFileSync(`public/models/${name}.glb`),g=await new GLTFLoader().setMeshoptDecoder(decoder).register(()=>({name:'CPU_QA_TEXTURES',loadTexture:()=>Promise.resolve(new T.Texture())})).parseAsync(b.buffer.slice(b.byteOffset,b.byteOffset+b.byteLength),'');const mixer=new T.AnimationMixer(g.scene);
 if(name==='human-sculpt-v3'){
  let checkedPaint=0;
  g.scene.traverse(o=>{
